...
df.sort_values(['id','start'], inplace=True, ascending=[False,True])
...
df.sort_values(['id', 'date', 'inning'], inplace=True, ascending=[False,True,True])
df.reset_index(inplace=True, drop=True)
...
#such that a rolling window of x days actually corresponds to x innnings
...
cols = ['IP','PA', 'AB', 'SAC', 'HBP', 'BB', 'S', 'D', 'T', 'HR', 'H', 'TB', 'SO', 'RBI', 'R', 'CS', 'SB', 'ER','W','L','QS']

##2021 NOTE all pitchers were using a 100 period averaging period!!!
df.sort_values(['id','date'], inplace=True, ascending=[False,True])
for col in cols:
    df[col+'_avg'] = df[[col,'id']].groupby('id').rolling(averaging_period, min_periods=0).sum().reset_index()[col]
...
df['pts_{0}'.format(averaging_period)] = (3*df['S_avg'] + 6*df['D_avg'] + 9*df['T_avg'] + 12*df['HR_avg']  +  3 * df['BB_avg'] + 3 * df['HBP_avg'] + 3.2 * df['R_avg'] + 3.5 * df['RBI_avg'] + 6 * df['SB_avg']) 
...
df['pts_{0}_parkadj'.format(averaging_period)] = df['pts_{0}'.format(averaging_period)] * df['park_ratio_SLG']
...
df[df['pts'].notnull()].to_pickle('./pitcher {0} 2021.pkl'.format(averaging_period))
