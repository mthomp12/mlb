...
averaging_period = 250
pw = password()
con = create_engine('mysql+mysqlconnector://{0}:{1}@localhost/mlb_db'.format(pw.user, pw.password))

df = pd.read_pickle('./batters {0} 2021.pkl'.format(averaging_period))
df2 = pd.read_pickle('./pitcher {0} 2021.pkl'.format(averaging_period))
...
df2.drop(['date','game','team','home','opp','number', 'park_factor_SLG_avg','park_factor_OBP_avg',
...
df['ops x pts_{0} parkadj'.format(averaging_period)] = df['OPS_avg_parkadj_p'] * df['pts_{0}_parkadj'.format(averaging_period)]
df['ops x pts_{0} orderadj'.format(averaging_period)] = df['OPS_avg_parkadj_p'] * df['pts_{0}_parkadj'.format(averaging_period)] / df['next_order']
df['ops23 x pts_{0}'.format(averaging_period)] = df['OPS_avg_parkadj_p']**(2/3) * df['pts_{0}_parkadj'.format(averaging_period)]
df['ops x pts_{0} order15'.format(averaging_period)] = df['OPS_avg_parkadj_p']**(2/3) * df['pts_{0}_parkadj'.format(averaging_period)] / df['next_order']**0.15

df['pts_BxP'] = df['pts_{0}_parkadj'.format(averaging_period)] * df['pts_{0}_parkadj_p'.format(averaging_period)]
...
df.to_pickle('ml_data {0} 2021.pkl'.format(averaging_period))
