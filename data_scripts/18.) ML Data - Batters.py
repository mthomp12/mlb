...
df.sort_values(['id', 'start', 'inning'], inplace=True, ascending=[False,True,True])
...
for col in ['game','team','opp','home','park_factor_SLG','park_factor_OBP','park']:
    df['next_{}'.format(col)] = df[['id',col]].groupby('id').shift(-1)
    

colStr = ', '.join(['date','number','team0','team1','pitcher0','pitcher1','away1','away2','away3','away4',
                    'away5','away6','away7','away8','away9','home1','home2','home3','home4','home5',
...
df['pts_{0}_parkadj'.format(averaging_period)] = df['pts_{}'.format(averaging_period)] * df['park_ratio_SLG']
...
df[df['pts'].notnull()].to_pickle('./batters {0} 2021.pkl'.format(averaging_period))
